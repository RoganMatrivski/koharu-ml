...
fn run() -> Result<()> {
    ...
    let llama_cpp_tag = env::var("LLAMA_CPP_TAG").unwrap_or_else(|_| "b4661".to_string());
...
fn validate_target() -> Result<()> {
    ...
    match target.as_str() {
        "x86_64-pc-windows-msvc" | "x86_64-unknown-linux-gnu" | "aarch64-unknown-linux-gnu" | "aarch64-apple-darwin" => {}
        _ => bail!(
            "unsupported target `{target}`; only Windows x86_64 MSVC, Linux x86_64, Linux aarch64, and macOS arm64 are supported"
        ),
    }
