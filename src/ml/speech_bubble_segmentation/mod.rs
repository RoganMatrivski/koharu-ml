...
use crate::ml::{device, loading, probability_map::ProbabilityMap};
...
impl SpeechBubbleSegmentation {
    pub async fn load(cpu: bool) -> Result<Self> {
        let (config_path, weights_path) = resolve_model_paths().await?;
...
pub async fn prefetch() -> Result<()> {
    let _ = resolve_model_paths().await?;
    Ok(())
}

async fn resolve_model_paths() -> Result<(PathBuf, PathBuf)> {
    let config = crate::ml::loading::hf_download(HF_REPO, CONFIG_FILENAME)
        .await
        .with_context(|| format!("failed to download {CONFIG_FILENAME} from {HF_REPO}"))?;
    let weights = crate::ml::loading::hf_download(HF_REPO, SAFETENSORS_FILENAME)
        .await
