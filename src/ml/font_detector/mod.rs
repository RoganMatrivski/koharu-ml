...
pub use crate::core::{FontPrediction, NamedFontPrediction, TextDirection, TopFont};
...
impl FontDetector {
    ...
    pub async fn load_with_kind(cpu: bool,
        kind: ModelKind,
    ) -> Result<Self> {
        let device = device(cpu)?;
        
        let weights_path = crate::ml::loading::hf_download(HF_REPO, "yuzumarker-font-detection.safetensors").await?;
...
impl FontLabels {
    pub async fn load() -> Result<Self> {
        let path = crate::ml::loading::hf_download(HF_REPO, "font-labels-ex.json").await?;
        Self::from_path(&path)
    }
