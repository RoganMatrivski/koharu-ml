...
use crate::ml::{Device, device, loading};
...
impl ComicTextBubbleDetector {
    pub async fn load(cpu: bool) -> Result<Self> {
        ...
        let config_path = loading::hf_download(HF_REPO, "config.json").await?;
        let preprocessor_path = loading::hf_download(HF_REPO, "preprocessor_config.json").await?;
        let weights_path = loading::hf_download(HF_REPO, "model.safetensors").await?;
