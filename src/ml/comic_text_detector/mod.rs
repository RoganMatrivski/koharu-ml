...
use crate::core::TextBlock;
use tracing::instrument;

use crate::ml::{device, loading};
...
impl ComicTextDetector {
    ...
    async fn load_inner(cpu: bool, load_dbnet: bool) -> anyhow::Result<Self> {
        ...
        let yolo_path = loading::hf_download(HF_REPO, "yolo-v5.safetensors").await?;
        let yolo = loading::load_mmaped_safetensors_path(&yolo_path, &device, |vb| {
            yolo_v5::YoloV5::load(vb, 2, 3)
        })?;
        let unet_path = loading::hf_download(HF_REPO, "unet.safetensors").await?;
        let unet = loading::load_mmaped_safetensors_path(&unet_path, &device, unet::UNet::load)?;
        let dbnet = if load_dbnet {
            let dbnet_path = loading::hf_download(HF_REPO, "dbnet.safetensors").await?;
...
pub async fn prefetch() -> anyhow::Result<()> {
    loading::hf_download(HF_REPO, "yolo-v5.safetensors").await?;
    loading::hf_download(HF_REPO, "unet.safetensors").await?;
    loading::hf_download(HF_REPO, "dbnet.safetensors").await?;
    Ok(())
}
