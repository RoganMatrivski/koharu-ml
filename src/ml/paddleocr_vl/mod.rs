...
impl PaddleOcrVl {
    pub async fn load(cpu: bool) -> Result<Self> {
        ...
        let files = ModelFiles {
            config: crate::ml::loading::hf_download(HF_REPO, "config.json").await?,
            preprocessor: crate::ml::loading::hf_download(HF_REPO, "preprocessor_config.json").await?,
            tokenizer: crate::ml::loading::hf_download(HF_REPO, "tokenizer.json").await?,
            weights: crate::ml::loading::hf_download(HF_REPO, "model.safetensors").await?,
        };
