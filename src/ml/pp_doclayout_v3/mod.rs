...
impl PPDocLayoutV3 {
    pub async fn load(cpu: bool) -> Result<Self> {
        ...
        let preprocessor_path = crate::ml::loading::hf_download(HF_REPO, "preprocessor_config.json").await?;
        ...
        let std = Tensor::from_slice(&preprocessor.image_std, (1, 3, 1, 1), &device)?
            .to_dtype(DType::F32)?;
        let weights_path = crate::ml::loading::hf_download(HF_REPO, "model.safetensors").await?;
