...
pub struct GgufContext {
    ctx: NonNull<crate::llm::sys::gguf_context>,
}
...
impl GgufContext {
    ...
    pub fn from_file(path: &Path) -> Option<Self> {
        ...
        let params = crate::llm::sys::gguf_init_params {
            no_alloc: true,
            ctx: std::ptr::null_mut(),
        };
        let ptr = unsafe { crate::llm::sys::gguf_init_from_file(c_path.as_ptr(), params) };
        Some(Self {
            ctx: NonNull::new(ptr)?,
        })
    }
    ...
    pub fn n_kv(&self) -> i64 {
        unsafe { crate::llm::sys::gguf_get_n_kv(self.ctx.as_ptr()) }
    }
    ...
    pub fn find_key(&self, key: &str) -> i64 {
        ...
        unsafe { crate::llm::sys::gguf_find_key(self.ctx.as_ptr(), c_key.as_ptr()) }
    }

    /// Return the key name at the given index, or `None` if out of range.
    pub fn key_at(&self, idx: i64) -> Option<&str> {
        let ptr = unsafe { crate::llm::sys::gguf_get_key(self.ctx.as_ptr(), idx) };
        if ptr.is_null() {
            return None;
        }
    ...
    pub fn kv_type(&self, idx: i64) -> crate::llm::sys::gguf_type {
        unsafe { crate::llm::sys::gguf_get_kv_type(self.ctx.as_ptr(), idx) }
    }
    ...
    pub fn val_u32(&self, idx: i64) -> u32 {
        unsafe { crate::llm::sys::gguf_get_val_u32(self.ctx.as_ptr(), idx) }
    }

    /// Read an `int32` value.
    pub fn val_i32(&self, idx: i64) -> i32 {
        unsafe { crate::llm::sys::gguf_get_val_i32(self.ctx.as_ptr(), idx) }
    }

    /// Read a `uint64` value.
    pub fn val_u64(&self, idx: i64) -> u64 {
        unsafe { crate::llm::sys::gguf_get_val_u64(self.ctx.as_ptr(), idx) }
    }

    /// Read a string value. Returns `None` if the pointer is null or not
    /// valid UTF-8.
    pub fn val_str(&self, idx: i64) -> Option<&str> {
        let ptr = unsafe { crate::llm::sys::gguf_get_val_str(self.ctx.as_ptr(), idx) };
        if ptr.is_null() {
            return None;
        }
        unsafe { CStr::from_ptr(ptr).to_str().ok() }
    ...
    pub fn n_tensors(&self) -> i64 {
        unsafe { crate::llm::sys::gguf_get_n_tensors(self.ctx.as_ptr()) }
    }
}

impl Drop for GgufContext {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::gguf_free(self.ctx.as_ptr()) }
    }
}
