...
use crate::llm::safe::token_type::{LlamaTokenAttr, LlamaTokenAttrs};
use crate::llm::safe::{
...
pub struct LlamaModel {
    pub(crate) model: NonNull<crate::llm::sys::llama_model>,
}
...
pub struct LlamaLoraAdapter {
    pub(crate) lora_adapter: NonNull<crate::llm::sys::llama_adapter_lora>,
}
...
impl LlamaModel {
    pub(crate) fn vocab_ptr(&self) -> *const crate::llm::sys::llama_vocab {
        unsafe { crate::llm::sys::llama_model_get_vocab(self.model.as_ptr()) }
    }
    ...
    pub fn n_ctx_train(&self) -> u32 {
        let n_ctx_train = unsafe { crate::llm::sys::llama_n_ctx_train(self.model.as_ptr()) };
    ...
    pub fn token_bos(&self) -> LlamaToken {
        let token = unsafe { crate::llm::sys::llama_token_bos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_eos(&self) -> LlamaToken {
        let token = unsafe { crate::llm::sys::llama_token_eos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_nl(&self) -> LlamaToken {
        let token = unsafe { crate::llm::sys::llama_token_nl(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn is_eog_token(&self, token: LlamaToken) -> bool {
        unsafe { crate::llm::sys::llama_token_is_eog(self.vocab_ptr(), token.0) }
    }

    /// Get the decoder start token.
    #[must_use]
    pub fn decode_start_token(&self) -> LlamaToken {
        let token = unsafe { crate::llm::sys::llama_model_decoder_start_token(self.model.as_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_sep(&self) -> LlamaToken {
        let token = unsafe { crate::llm::sys::llama_vocab_sep(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn str_to_token(
        ...
        let size = unsafe {
            crate::llm::sys::llama_tokenize(
                self.vocab_ptr(),
                c_string.as_ptr(),
                c_int::try_from(c_string.as_bytes().len())?,
                buffer.as_mut_ptr().cast::<crate::llm::sys::llama_token>(),
                buffer_capacity,
                add_bos,
                true,
            )
    ...
    pub fn token_attr(&self, LlamaToken(id): LlamaToken) -> LlamaTokenAttrs {
        let token_type = unsafe { crate::llm::sys::llama_token_get_attr(self.vocab_ptr(), id) };
    ...
    pub fn token_to_piece_bytes(
        ...
        let size = unsafe {
            crate::llm::sys::llama_token_to_piece(self.vocab_ptr(), token.0, buf, len, lstrip, special)
        };
    ...
    pub fn n_vocab(&self) -> i32 {
        unsafe { crate::llm::sys::llama_n_vocab(self.vocab_ptr()) }
    }
    ...
    pub fn vocab_type(&self) -> VocabType {
        // crate::llm::sys::llama_model_get_vocab
        let vocab_type = unsafe { crate::llm::sys::llama_vocab_type(self.vocab_ptr()) };
    ...
    pub fn n_embd(&self) -> c_int {
        unsafe { crate::llm::sys::llama_n_embd(self.model.as_ptr()) }
    }

    /// Returns the total size of all the tensors in the model in bytes.
    pub fn size(&self) -> u64 {
        unsafe { crate::llm::sys::llama_model_size(self.model.as_ptr()) }
    }

    /// Returns the number of parameters in the model.
    pub fn n_params(&self) -> u64 {
        unsafe { crate::llm::sys::llama_model_n_params(self.model.as_ptr()) }
    }

    /// Returns whether the model is a recurrent network (Mamba, RWKV, etc)
    pub fn is_recurrent(&self) -> bool {
        unsafe { crate::llm::sys::llama_model_is_recurrent(self.model.as_ptr()) }
    }
    ...
    pub fn n_layer(&self) -> u32 {
        ...
        u32::try_from(unsafe { crate::llm::sys::llama_model_n_layer(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn n_head(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { crate::llm::sys::llama_model_n_head(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn n_head_kv(&self) -> u32 {
        // It's never possible for this to panic because while the API interface is defined as an int32_t,
        // the field it's accessing is a uint32_t.
        u32::try_from(unsafe { crate::llm::sys::llama_model_n_head_kv(self.model.as_ptr()) }).unwrap()
    }
    ...
    pub fn meta_val_str(&self, key: &str) -> Result<String, MetaValError> {
        ...
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                crate::llm::sys::llama_model_meta_val_str(self.model.as_ptr(), key_ptr, buf_ptr, buf_len)
            },
            256,
        )
    }
    ...
    pub fn meta_count(&self) -> i32 {
        unsafe { crate::llm::sys::llama_model_meta_count(self.model.as_ptr()) }
    }
    ...
    pub fn meta_key_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                crate::llm::sys::llama_model_meta_key_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_val_str_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                crate::llm::sys::llama_model_meta_val_str_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn rope_type(&self) -> Option<RopeType> {
        match unsafe { crate::llm::sys::llama_model_rope_type(self.model.as_ptr()) } {
            crate::llm::sys::LLAMA_ROPE_TYPE_NONE => None,
            crate::llm::sys::LLAMA_ROPE_TYPE_NORM => Some(RopeType::Norm),
            crate::llm::sys::LLAMA_ROPE_TYPE_NEOX => Some(RopeType::NeoX),
            crate::llm::sys::LLAMA_ROPE_TYPE_MROPE => Some(RopeType::MRope),
            crate::llm::sys::LLAMA_ROPE_TYPE_VISION => Some(RopeType::Vision),
            rope_type => {
    ...
    pub fn chat_template(
        ...
        let result =
            unsafe { crate::llm::sys::llama_model_chat_template(self.model.as_ptr(), name_ptr) };
    ...
    pub fn load_from_file(
        ...
        let llama_model =
            unsafe { crate::llm::sys::llama_load_model_from_file(cstr.as_ptr(), params.params) };
    ...
    pub fn lora_adapter_init(
        ...
        let cstr = CString::new(path)?;
        let adapter =
            unsafe { crate::llm::sys::llama_adapter_lora_init(self.model.as_ptr(), cstr.as_ptr()) };
    ...
    pub fn new_context<'a>(
        ...
        let context = unsafe {
            crate::llm::sys::llama_new_context_with_model(self.model.as_ptr(), context_params)
        };
    ...
    pub fn apply_chat_template(
        ...
        let chat: Vec<crate::llm::sys::llama_chat_message> = chat
            .iter()
            .map(|c| crate::llm::sys::llama_chat_message {
                role: c.role.as_ptr(),
        ...
        let res = unsafe {
            crate::llm::sys::llama_chat_apply_template(
                tmpl_ptr,
                chat.as_ptr(),
                chat.len(),
                add_ass,
...
impl Drop for LlamaModel {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::llama_free_model(self.model.as_ptr()) }
    }
}
...
pub enum VocabType {
    /// Byte Pair Encoding
    BPE = crate::llm::sys::LLAMA_VOCAB_TYPE_BPE as _,
    /// Sentence Piece Tokenizer
    SPM = crate::llm::sys::LLAMA_VOCAB_TYPE_SPM as _,
}
...
pub enum LlamaTokenTypeFromIntError {
    ...
    UnknownValue(crate::llm::sys::llama_vocab_type),
}

impl TryFrom<crate::llm::sys::llama_vocab_type> for VocabType {
    type Error = LlamaTokenTypeFromIntError;

    fn try_from(value: crate::llm::sys::llama_vocab_type) -> Result<Self, Self::Error> {
        match value {
            crate::llm::sys::LLAMA_VOCAB_TYPE_BPE => Ok(VocabType::BPE),
            crate::llm::sys::LLAMA_VOCAB_TYPE_SPM => Ok(VocabType::SPM),
