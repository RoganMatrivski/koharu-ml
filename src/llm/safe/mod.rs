...
use crate::llm::safe::llama_batch::BatchAddError;
...
pub fn llama_time_us() -> i64 {
    unsafe { crate::llm::sys::llama_time_us() }
}
...
pub fn max_devices() -> usize {
    unsafe { crate::llm::sys::llama_max_devices() }
}
...
pub fn ggml_time_us() -> i64 {
    unsafe { crate::llm::sys::ggml_time_us() }
}
...
pub fn list_llama_ggml_backend_devices() -> Vec<LlamaBackendDevice> {
    ...
    for i in 0..unsafe { crate::llm::sys::ggml_backend_dev_count() } {
        ...
        let props = unsafe {
            ...
            crate::llm::sys::ggml_backend_dev_get_props(dev, &raw mut props);
            props
        };
        let name = cstr_to_string(props.name);
        let description = cstr_to_string(props.description);
        let backend = unsafe { crate::llm::sys::ggml_backend_dev_backend_reg(dev) };
        let backend_name = unsafe { crate::llm::sys::ggml_backend_reg_name(backend) };
        ...
        let device_type = match props.type_ {
            crate::llm::sys::GGML_BACKEND_DEVICE_TYPE_CPU => LlamaBackendDeviceType::Cpu,
            crate::llm::sys::GGML_BACKEND_DEVICE_TYPE_ACCEL => LlamaBackendDeviceType::Accelerator,
            crate::llm::sys::GGML_BACKEND_DEVICE_TYPE_GPU => LlamaBackendDeviceType::Gpu,
            crate::llm::sys::GGML_BACKEND_DEVICE_TYPE_IGPU => LlamaBackendDeviceType::IntegratedGpu,
...
pub fn send_logs_to_tracing(options: LogOptions) {
    ...
    unsafe {
        ...
        crate::llm::sys::llama_log_set(Some(logs_to_trace), llama_heap_state as *mut _);
        crate::llm::sys::ggml_log_set(Some(logs_to_trace), ggml_heap_state as *mut _);
    }
}
