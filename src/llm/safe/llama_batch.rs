...
use crate::llm::sys::{llama_batch, llama_batch_free, llama_batch_init, llama_pos, llama_seq_id};
...
pub struct LlamaBatch<'a> {
    ...
    /// The llama_cpp batch. always initialize by `crate::llm::sys::llama_batch_init(allocated, <unknown>, <unknown>)`
...
impl<'a> LlamaBatch<'a> {
    ...
    pub fn get_one(tokens: &'a [LlamaToken]) -> Result<Self, BatchAddError> {
        ...
        let batch = unsafe {
            ...
            crate::llm::sys::llama_batch_get_one(
                ptr,
                tokens
                    .len()
                    .try_into()
