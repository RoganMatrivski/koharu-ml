...
use crate::llm::safe::context::LlamaContext;
use crate::llm::safe::model::LlamaModel;
...
/// assert_eq!(text_chunk, crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_TEXT.into());
...
pub enum MtmdInputChunkType {
    /// Text input chunk
    Text = crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_TEXT as _,
    /// Image input chunk
    Image = crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_IMAGE as _,
    /// Audio input chunk
    Audio = crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_AUDIO as _,
}

impl From<crate::llm::sys::mtmd_input_chunk_type> for MtmdInputChunkType {
    fn from(chunk_type: crate::llm::sys::mtmd_input_chunk_type) -> Self {
        match chunk_type {
            crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_TEXT => MtmdInputChunkType::Text,
            crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_IMAGE => MtmdInputChunkType::Image,
            crate::llm::sys::MTMD_INPUT_CHUNK_TYPE_AUDIO => MtmdInputChunkType::Audio,
...
impl Default for MtmdContextParams {
    fn default() -> Self {
        unsafe { crate::llm::sys::mtmd_context_params_default() }.into()
    }
}

impl From<&MtmdContextParams> for crate::llm::sys::mtmd_context_params {
    fn from(params: &MtmdContextParams) -> Self {
        let mut context = unsafe { crate::llm::sys::mtmd_context_params_default() };
...
impl From<crate::llm::sys::mtmd_context_params> for MtmdContextParams {
    fn from(params: crate::llm::sys::mtmd_context_params) -> Self {
        Self {
...
pub struct MtmdContext {
    pub(crate) context: NonNull<crate::llm::sys::mtmd_context>,
}
...
impl MtmdContext {
    ...
    pub fn init_from_file(
        ...
        let ctx_params = crate::llm::sys::mtmd_context_params::from(params);

        let context = unsafe {
            crate::llm::sys::mtmd_init_from_file(
                path_cstr.as_ptr(),
    ...
    pub fn decode_use_non_causal(&self) -> bool {
        unsafe { crate::llm::sys::mtmd_decode_use_non_causal(self.context.as_ptr()) }
    }
    ...
    pub fn decode_use_mrope(&self) -> bool {
        unsafe { crate::llm::sys::mtmd_decode_use_mrope(self.context.as_ptr()) }
    }

    /// Check whether the current model supports vision input.
    #[must_use]
    pub fn support_vision(&self) -> bool {
        unsafe { crate::llm::sys::mtmd_support_vision(self.context.as_ptr()) }
    }

    /// Check whether the current model supports audio input.
    #[must_use]
    pub fn support_audio(&self) -> bool {
        unsafe { crate::llm::sys::mtmd_support_audio(self.context.as_ptr()) }
    }
    ...
    pub fn get_audio_sample_rate(&self) -> Option<u32> {
        let rate = unsafe { crate::llm::sys::mtmd_get_audio_sample_rate(self.context.as_ptr()) };
    ...
    pub fn tokenize(
        ...
        let input_text = crate::llm::sys::mtmd_input_text {
        ...
        let bitmap_ptrs: Vec<*const crate::llm::sys::mtmd_bitmap> = bitmaps
            .iter()
            .map(|b| b.bitmap.as_ptr().cast_const())
            .collect();

        let result = unsafe {
            crate::llm::sys::mtmd_tokenize(
                self.context.as_ptr(),
    ...
    pub fn encode_chunk(&self, chunk: &MtmdInputChunk) -> Result<(), MtmdEncodeError> {
        let result =
            unsafe { crate::llm::sys::mtmd_encode_chunk(self.context.as_ptr(), chunk.chunk.as_ptr()) };
...
impl Drop for MtmdContext {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::mtmd_free(self.context.as_ptr()) }
    }
}
...
pub struct MtmdBitmap {
    pub(crate) bitmap: NonNull<crate::llm::sys::mtmd_bitmap>,
}
...
impl MtmdBitmap {
    ...
    pub fn from_image_data(nx: u32, ny: u32, data: &[u8]) -> Result<Self, MtmdBitmapError> {
        ...
        let bitmap = unsafe { crate::llm::sys::mtmd_bitmap_init(nx, ny, data.as_ptr()) };
    ...
    pub fn from_audio_data(data: &[f32]) -> Result<Self, MtmdBitmapError> {
        let bitmap = unsafe { crate::llm::sys::mtmd_bitmap_init_from_audio(data.len(), data.as_ptr()) };
    ...
    pub fn from_file(ctx: &MtmdContext, path: &str) -> Result<Self, MtmdBitmapError> {
        ...
        let bitmap = unsafe {
            crate::llm::sys::mtmd_helper_bitmap_init_from_file(ctx.context.as_ptr(), path_cstr.as_ptr())
        };
    ...
    pub fn from_buffer(ctx: &MtmdContext, data: &[u8]) -> Result<Self, MtmdBitmapError> {
        let bitmap = unsafe {
            crate::llm::sys::mtmd_helper_bitmap_init_from_buf(
                ctx.context.as_ptr(),
                data.as_ptr(),
                data.len(),
            )
    ...
    #[must_use]
    pub fn nx(&self) -> u32 {
        unsafe { crate::llm::sys::mtmd_bitmap_get_nx(self.bitmap.as_ptr()) }
    }

    /// Get bitmap height in pixels.
    #[must_use]
    pub fn ny(&self) -> u32 {
        unsafe { crate::llm::sys::mtmd_bitmap_get_ny(self.bitmap.as_ptr()) }
    }
    ...
    #[must_use]
    pub fn data(&self) -> &[u8] {
        let ptr = unsafe { crate::llm::sys::mtmd_bitmap_get_data(self.bitmap.as_ptr()) };
        let len = unsafe { crate::llm::sys::mtmd_bitmap_get_n_bytes(self.bitmap.as_ptr()) };
    ...
    pub fn is_audio(&self) -> bool {
        unsafe { crate::llm::sys::mtmd_bitmap_is_audio(self.bitmap.as_ptr()) }
    }
    ...
    pub fn id(&self) -> Option<String> {
        let ptr = unsafe { crate::llm::sys::mtmd_bitmap_get_id(self.bitmap.as_ptr()) };
        if ptr.is_null() {
            None
        } else {
    ...
    pub fn set_id(&self, id: &str) -> Result<(), std::ffi::NulError> {
        ...
        unsafe {
            crate::llm::sys::mtmd_bitmap_set_id(self.bitmap.as_ptr(), id_cstr.as_ptr());
        }
        Ok(())
    }
}
...
impl Drop for MtmdBitmap {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::mtmd_bitmap_free(self.bitmap.as_ptr()) }
    }
}
...
pub struct MtmdInputChunks {
    pub(crate) chunks: NonNull<crate::llm::sys::mtmd_input_chunks>,
}
...
impl MtmdInputChunks {
    ...
    #[must_use]
    pub fn new() -> Self {
        let chunks = unsafe { crate::llm::sys::mtmd_input_chunks_init() };
    ...
    pub fn len(&self) -> usize {
        unsafe { crate::llm::sys::mtmd_input_chunks_size(self.chunks.as_ptr()) }
    }
    ...
    pub fn get(&self, index: usize) -> Option<MtmdInputChunk> {
        ...
        let chunk_ptr = unsafe { crate::llm::sys::mtmd_input_chunks_get(self.chunks.as_ptr(), index) };
    ...
    pub fn total_tokens(&self) -> usize {
        unsafe { crate::llm::sys::mtmd_helper_get_n_tokens(self.chunks.as_ptr()) }
    }
    ...
    pub fn total_positions(&self) -> i32 {
        unsafe { crate::llm::sys::mtmd_helper_get_n_pos(self.chunks.as_ptr()) }
    }
    ...
    pub fn eval_chunks(
        ...
        n_past: crate::llm::sys::llama_pos,
        seq_id: crate::llm::sys::llama_seq_id,
        n_batch: i32,
        logits_last: bool,
    ) -> Result<crate::llm::sys::llama_pos, MtmdEvalError> {
        let mut new_n_past: crate::llm::sys::llama_pos = 0;

        let result = unsafe {
            crate::llm::sys::mtmd_helper_eval_chunks(
...
impl Drop for MtmdInputChunks {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::mtmd_input_chunks_free(self.chunks.as_ptr()) }
    }
}
...
pub struct MtmdInputChunk {
    pub(crate) chunk: NonNull<crate::llm::sys::mtmd_input_chunk>,
    owned: bool,
}
...
impl MtmdInputChunk {
    ...
    pub fn chunk_type(&self) -> MtmdInputChunkType {
        let chunk_type = unsafe { crate::llm::sys::mtmd_input_chunk_get_type(self.chunk.as_ptr()) };
    ...
    pub fn text_tokens(&self) -> Option<&[LlamaToken]> {
        ...
        let tokens_ptr = unsafe {
            crate::llm::sys::mtmd_input_chunk_get_tokens_text(self.chunk.as_ptr(), &raw mut n_tokens)
        };
    ...
    pub fn n_tokens(&self) -> usize {
        unsafe { crate::llm::sys::mtmd_input_chunk_get_n_tokens(self.chunk.as_ptr()) }
    }
    ...
    pub fn n_positions(&self) -> i32 {
        unsafe { crate::llm::sys::mtmd_input_chunk_get_n_pos(self.chunk.as_ptr()) }
    }
    ...
    #[must_use]
    pub fn id(&self) -> Option<String> {
        let ptr = unsafe { crate::llm::sys::mtmd_input_chunk_get_id(self.chunk.as_ptr()) };
        if ptr.is_null() {
            None
        } else {
    ...
    pub fn copy(&self) -> Result<Self, MtmdInputChunkError> {
        let chunk = unsafe { crate::llm::sys::mtmd_input_chunk_copy(self.chunk.as_ptr()) };
...
impl Drop for MtmdInputChunk {
    fn drop(&mut self) {
        if self.owned {
            unsafe { crate::llm::sys::mtmd_input_chunk_free(self.chunk.as_ptr()) }
        }
    }
}
...
pub fn mtmd_default_marker() -> &'static str {
    unsafe {
        let c_str = crate::llm::sys::mtmd_default_marker();
