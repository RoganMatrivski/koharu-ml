...
use crate::llm::safe::model::{LlamaLoraAdapter, LlamaModel};
use crate::llm::safe::timing::LlamaTimings;
use crate::llm::safe::token::LlamaToken;
use crate::llm::safe::token::data::LlamaTokenData;
...
#[path = "context/kv_cache.rs"]
pub mod kv_cache;
#[path = "context/params.rs"]
pub mod params;
...
pub struct LlamaContext<'a> {
    pub(crate) context: NonNull<crate::llm::sys::llama_context>,
...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        llama_context: NonNull<crate::llm::sys::llama_context>,
        embeddings_enabled: bool,
    ) -> Self {
        Self {
    ...
    pub fn n_batch(&self) -> u32 {
        unsafe { crate::llm::sys::llama_n_batch(self.context.as_ptr()) }
    }

    /// Gets the max number of physical tokens (hardware level) to decode in batch. Must be less than or equal to [`Self::n_batch`].
    #[must_use]
    pub fn n_ubatch(&self) -> u32 {
        unsafe { crate::llm::sys::llama_n_ubatch(self.context.as_ptr()) }
    }

    /// Gets the size of the context.
    #[must_use]
    pub fn n_ctx(&self) -> u32 {
        unsafe { crate::llm::sys::llama_n_ctx(self.context.as_ptr()) }
    }
    ...
    pub fn decode(&mut self, batch: &mut LlamaBatch) -> Result<(), DecodeError> {
        let result = unsafe { crate::llm::sys::llama_decode(self.context.as_ptr(), batch.llama_batch) };
    ...
    pub fn encode(&mut self, batch: &mut LlamaBatch) -> Result<(), EncodeError> {
        let result = unsafe { crate::llm::sys::llama_encode(self.context.as_ptr(), batch.llama_batch) };
    ...
    /// - If the current model had a pooling type of [`crate::llm::sys::LLAMA_POOLING_TYPE_NONE`]
    ...
    pub fn embeddings_seq_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = crate::llm::sys::llama_get_embeddings_seq(self.context.as_ptr(), i);
    ...
    pub fn embeddings_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = crate::llm::sys::llama_get_embeddings_ith(self.context.as_ptr(), i);
    ...
    pub fn get_logits(&self) -> &[f32] {
        let data = unsafe { crate::llm::sys::llama_get_logits(self.context.as_ptr()) };
    ...
    pub fn get_logits_ith(&self, i: i32) -> &[f32] {
        ...
        let data = unsafe { crate::llm::sys::llama_get_logits_ith(self.context.as_ptr(), i) };
        let len = usize::try_from(self.model.n_vocab()).expect("n_vocab does not fit into a usize");
    ...
    pub fn reset_timings(&mut self) {
        unsafe { crate::llm::sys::llama_perf_context_reset(self.context.as_ptr()) }
    }

    /// Returns the timings for the context.
    pub fn timings(&mut self) -> LlamaTimings {
        let timings = unsafe { crate::llm::sys::llama_perf_context(self.context.as_ptr()) };
    ...
    pub fn lora_adapter_set(
        ...
        let err_code = unsafe {
            crate::llm::sys::llama_set_adapters_lora(
                self.context.as_ptr(),
                adapters.as_mut_ptr(),
                1,
                scales.as_mut_ptr(),
    ...
    pub fn print_memory_breakdown(&self) {
        unsafe { crate::llm::sys::llama_memory_breakdown_print(self.context.as_ptr()) }
    }
}

impl Drop for LlamaContext<'_> {
    fn drop(&mut self) {
        unsafe { crate::llm::sys::llama_free(self.context.as_ptr()) }
    }
}
