...
use crate::llm::safe::{sampling::LlamaSampler, token::data::LlamaTokenData};
...
impl LlamaTokenDataArray {
    ...
    pub(crate) unsafe fn modify_as_c_llama_token_data_array<T>(
        &mut self,
        modify: impl FnOnce(&mut crate::llm::sys::llama_token_data_array) -> T,
    ) -> T {
        let size = self.data.len();
        let data = self
            .data
            .as_mut_ptr()
            .cast::<crate::llm::sys::llama_token_data>();

        let mut c_llama_token_data_array = crate::llm::sys::llama_token_data_array {
            data,
            size,
    ...
    pub fn apply_sampler(&mut self, sampler: &LlamaSampler) {
        unsafe {
            self.modify_as_c_llama_token_data_array(|c_llama_token_data_array| {
                crate::llm::sys::llama_sampler_apply(sampler.sampler, c_llama_token_data_array);
            });
        }
    }
