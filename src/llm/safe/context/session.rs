...
impl LlamaContext<'_> {
    ...
    pub fn save_session_file(
        ...
        if unsafe {
            crate::llm::sys::llama_save_session_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens.as_ptr().cast::<crate::llm::sys::llama_token>(),
                tokens.len(),
            )
        } {
            Ok(())
    ...
    pub fn load_session_file(
        ...
        let tokens_out = tokens.as_mut_ptr().cast::<crate::llm::sys::llama_token>();

        let load_session_success = unsafe {
            crate::llm::sys::llama_load_session_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens_out,
                max_tokens,
    ...
    pub fn state_save_file(
        ...
        if unsafe {
            crate::llm::sys::llama_state_save_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens.as_ptr().cast::<crate::llm::sys::llama_token>(),
                tokens.len(),
    ...
    pub fn state_load_file(
        ...
        let success = unsafe {
            crate::llm::sys::llama_state_load_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens_out,
                max_tokens,
    ...
    pub fn state_seq_save_file(
        ...
        let bytes_written = unsafe {
            crate::llm::sys::llama_state_seq_save_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                seq_id,
                tokens.as_ptr().cast::<crate::llm::sys::llama_token>(),
    ...
    pub fn state_seq_load_file(
        ...
        let bytes_read = unsafe {
            crate::llm::sys::llama_state_seq_load_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                dest_seq_id,
                tokens_out,
    ...
    pub fn get_state_size(&self) -> usize {
        unsafe { crate::llm::sys::llama_get_state_size(self.context.as_ptr()) }
    }
    ...
    pub unsafe fn copy_state_data(&self, dest: *mut u8) -> usize {
        unsafe { crate::llm::sys::llama_copy_state_data(self.context.as_ptr(), dest) }
    }
    ...
    pub unsafe fn set_state_data(&mut self, src: &[u8]) -> usize {
        unsafe { crate::llm::sys::llama_set_state_data(self.context.as_ptr(), src.as_ptr()) }
    }
}
