...
fn meta_for_level(
    level: crate::llm::sys::ggml_log_level,
) -> (&'static Metadata<'static>, &'static OverridableFields) {
    match level {
        crate::llm::sys::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        crate::llm::sys::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        crate::llm::sys::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        crate::llm::sys::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(crate::llm::sys::ggml_log_level, String)>>,
...
impl State {
    ...
    fn generate_log(target: Module, level: crate::llm::sys::ggml_log_level, text: &str) {
    ...
    pub(super) fn cont_buffered_log(&self, text: &str) {
        ...
        if let Some((previous_log_level, mut buffer)) = lock.take() {
            ...
            let level = self
                ...
                as crate::llm::sys::ggml_log_level;
            tracing::warn!(
    ...
    pub(super) fn buffer_non_cont(&self, level: crate::llm::sys::ggml_log_level, text: &str) {
        debug_assert!(!text.ends_with('\n'));
        debug_assert_ne!(level, crate::llm::sys::GGML_LOG_LEVEL_CONT);
    ...
    #[allow(clippy::unnecessary_cast)]
    pub(super) fn emit_non_cont_line(&self, level: crate::llm::sys::ggml_log_level, text: &str) {
        debug_assert!(text.ends_with('\n'));
        debug_assert_ne!(level, crate::llm::sys::GGML_LOG_LEVEL_CONT);
        ...
        match level {
            crate::llm::sys::GGML_LOG_LEVEL_NONE => {
                // TODO: Support logging this to stdout directly via options?
                tracing::info!(no_log_level = true, text);
            }
            crate::llm::sys::GGML_LOG_LEVEL_DEBUG
            | crate::llm::sys::GGML_LOG_LEVEL_INFO
            | crate::llm::sys::GGML_LOG_LEVEL_WARN
            | crate::llm::sys::GGML_LOG_LEVEL_ERROR => Self::generate_log(self.module, level, text),
            crate::llm::sys::GGML_LOG_LEVEL_CONT => unreachable!(),
            _ => {
                tracing::warn!(
                    level = level,
                    text = text,
    ...
    #[allow(clippy::unnecessary_cast)]
    pub(super) fn update_previous_level_for_disabled_log(&self, level: crate::llm::sys::ggml_log_level) {
        if level != crate::llm::sys::GGML_LOG_LEVEL_CONT {
            self.previous_level
                .store(level as i32, std::sync::atomic::Ordering::Release);
        }
    }
    ...
    pub(super) fn is_enabled_for_level(&self, level: crate::llm::sys::ggml_log_level) -> bool {
        // CONT logs do not need to check if they are enabled.
        let level = if level == crate::llm::sys::GGML_LOG_LEVEL_CONT {
            self.previous_level
...
mod tests {
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(
            crate::llm::sys::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
        );
        ...
        logs_to_trace(crate::llm::sys::GGML_LOG_LEVEL_CONT, c"world".as_ptr(), log_ptr);
        logs_to_trace(crate::llm::sys::GGML_LOG_LEVEL_CONT, c"\n".as_ptr(), log_ptr);
    }
    ...
    fn cont_enabled_log() {
        ...
        logs_to_trace(crate::llm::sys::GGML_LOG_LEVEL_INFO, c"Hello ".as_ptr(), log_ptr);
        logs_to_trace(
            crate::llm::sys::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
