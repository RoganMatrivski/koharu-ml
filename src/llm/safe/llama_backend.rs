...
use crate::llm::safe::LlamaCppError;
use crate::llm::sys::ggml_log_level;
...
impl LlamaBackend {
    ...
    fn mark_init() -> crate::llm::safe::Result<()> {
    ...
    pub fn init() -> crate::llm::safe::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { crate::llm::sys::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    /// ```
    #[tracing::instrument(skip_all)]
    pub fn init_numa(strategy: NumaStrategy) -> crate::llm::safe::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            crate::llm::sys::llama_numa_init(crate::llm::sys::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            crate::llm::sys::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub crate::llm::sys::ggml_numa_strategy,
);

impl TryFrom<crate::llm::sys::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: crate::llm::sys::ggml_numa_strategy) -> std::result::Result<Self, Self::Error> {
        match value {
            crate::llm::sys::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            crate::llm::sys::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            crate::llm::sys::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            crate::llm::sys::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            crate::llm::sys::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            crate::llm::sys::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for crate::llm::sys::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => crate::llm::sys::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => crate::llm::sys::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => crate::llm::sys::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => crate::llm::sys::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => crate::llm::sys::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => crate::llm::sys::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { crate::llm::sys::llama_backend_free() }
    }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = crate::llm::sys::ggml_numa_strategy::from(*numa);
