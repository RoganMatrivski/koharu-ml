...
use crate::llm::safe::token::data_array::LlamaTokenDataArray;
use crate::llm::safe::token::logit_bias::LlamaLogitBias;
use crate::llm::safe::{GrammarError, SamplerAcceptError};

/// A safe wrapper around `llama_sampler`.
pub struct LlamaSampler {
    pub(crate) sampler: *mut crate::llm::sys::llama_sampler,
}
...
impl LlamaSampler {
    ...
    pub fn sample(&mut self, ctx: &LlamaContext, idx: i32) -> LlamaToken {
        let token =
            unsafe { crate::llm::sys::llama_sampler_sample(self.sampler, ctx.context.as_ptr(), idx) };
    ...
    pub fn try_accept(&mut self, token: LlamaToken) -> Result<(), SamplerAcceptError> {
        unsafe {
            crate::llm::sys::llama_sampler_accept(self.sampler, token.0);
        }
        Ok(())
    }
    ...
    pub fn reset(&mut self) {
        unsafe {
            crate::llm::sys::llama_sampler_reset(self.sampler);
        }
    }
    ...
    pub fn get_seed(&self) -> u32 {
        unsafe { crate::llm::sys::llama_sampler_get_seed(self.sampler) }
    }
    ...
    pub fn chain(samplers: impl IntoIterator<Item = Self>, no_perf: bool) -> Self {
        unsafe {
            let chain =
                crate::llm::sys::llama_sampler_chain_init(crate::llm::sys::llama_sampler_chain_params {
                    no_perf,
                });

            for sampler in samplers {
                crate::llm::sys::llama_sampler_chain_add(chain, sampler.sampler);
    ...
    pub fn temp(t: f32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_temp(t) };
        Self { sampler }
    }
    ...
    pub fn temp_ext(t: f32, delta: f32, exponent: f32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_temp_ext(t, delta, exponent) };
        Self { sampler }
    }
    ...
    pub fn top_k(k: i32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_top_k(k) };
        Self { sampler }
    }
    ...
    pub fn top_n_sigma(n: f32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_top_n_sigma(n) };
        Self { sampler }
    }
    ...
    pub fn typical(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_typical(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn top_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_top_p(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn min_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_min_p(p, min_keep) };
        Self { sampler }
    }
    ...
    pub fn xtc(p: f32, t: f32, min_keep: usize, seed: u32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_xtc(p, t, min_keep, seed) };
        Self { sampler }
    }
    ...
    pub fn grammar(
        ...
        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_grammar(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
            )
    ...
    pub fn grammar_lazy(
        ...
        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_grammar_lazy(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
    ...
    pub fn grammar_lazy_patterns(
        ...
        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_grammar_lazy_patterns(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
    ...
    pub fn dry(
        ...
        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_dry(
                model.vocab_ptr(),
                model
                    .n_ctx_train()
                    .try_into()
    ...
    pub fn penalties(
        ...
        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_penalties(
                penalty_last_n,
                penalty_repeat,
                penalty_freq,
                penalty_present,
    ...
    pub fn mirostat(n_vocab: i32, seed: u32, tau: f32, eta: f32, m: i32) -> Self {
        let sampler =
            unsafe { crate::llm::sys::llama_sampler_init_mirostat(n_vocab, seed, tau, eta, m) };
        Self { sampler }
    }
    ...
    pub fn mirostat_v2(seed: u32, tau: f32, eta: f32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_mirostat_v2(seed, tau, eta) };
        Self { sampler }
    }
    ...
    pub fn dist(seed: u32) -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_dist(seed) };
        Self { sampler }
    }
    ...
    #[must_use]
    pub fn greedy() -> Self {
        let sampler = unsafe { crate::llm::sys::llama_sampler_init_greedy() };
        Self { sampler }
    }
    ...
    pub fn logit_bias(n_vocab: i32, biases: &[LlamaLogitBias]) -> Self {
        let data = biases.as_ptr().cast::<crate::llm::sys::llama_logit_bias>();

        let sampler = unsafe {
            crate::llm::sys::llama_sampler_init_logit_bias(n_vocab, biases.len() as i32, data)
        };
...
impl Drop for LlamaSampler {
    fn drop(&mut self) {
        unsafe {
            crate::llm::sys::llama_sampler_free(self.sampler);
        }
    }
}
